...
- **下载入口与传输中心**：状态栏右侧稳定显示“↓ 下载”；必要时显示整体进度、短暂完成
  提示或未查看失败。单个文件落盘后立即刷新到主面板，图片质量比对与重复清理继续在
  后台完成；传输中心按批次保留选择，并显示完成进度及平均传输速度
...
- **可选默认右键菜单**：可在共享设置中选择 PopDrop 快捷菜单或完整 Windows 系统菜单
- **统一 Shift 反转**：右键打开默认菜单；按住 Shift 右键或按 `Shift + F10` 打开另一个菜单
- **键盘操作**：`Enter` 打开、`Delete` 移入回收站、`Ctrl + Enter` 定位、`Ctrl + C` 复制文件对象、`Ctrl + Shift + C` 复制路径
- **可选单击打开**：普通文件可全局或按来源切换单击/双击；多选、拖拽和文件夹双击不变
- **应用与工具操作**：一个应用可同时提供按扩展名的“打开方式”和多个带独立参数、
  适用条件及执行程序的工具动作
...
可管理通用选项、当前工作区来源、应用与工具动作、复制/移动常用位置和排除规则。只有高级选项才需要直接编辑同目录下的
...
“共享设置 → 通用 → 右键菜单”可以选择默认打开 PopDrop 快捷菜单或 Windows 系统
菜单。右键和菜单键打开默认菜单；按住 Shift 右键或按 `Shift + F10` 打开另一个菜单。
...
在“共享设置 → 打开与文件操作”中，一个应用只保存一次主程序路径，并可分别设置：
是否显示为普通文件的“打开方式”，以及零个或多个工具动作。打开方式仍只处理单个
普通文件；工具动作可以按文件、文件夹、扩展名和共同文件夹动态显示，并选择“逐个项目
串行执行”或“一次传入全部项目”。
动作按应用顺序、再按应用内顺序进入右键菜单，适用的前 5 个直接显示，其余进入
“更多工具操作…”。

每个动作的参数模板一行一个参数，支持 `{item}`、`{items}`、`{folder}`、`{parent}`、
`{name}`、`{stem}`、`{ext}`、日期时间、序号、总数和大小变量。`{items}` 仅用于
“一次传入全部项目”，必须单独占一行并展开成多个独立路径参数；逐个模式在后台等待
当前外部进程退出后再启动下一项，不阻塞 PopDrop 界面。应用主 EXE、动作覆盖 EXE、
参数列表和工作目录始终分开传给 Windows；不经过 `cmd.exe`，也不支持脚本或 Shell
管道。完整配置和 7-Zip 示例见
**[使用指南](USAGE.md#应用与工具操作)**。
...
程序路径、动作参数列表和工作目录分别交给 Windows API；只允许配置现有 `.exe`，
不会把动作交给命令解释器。Windows 接受启动请求只表示外部工具已启动，不代表压缩、
解压或转换已经完成。软件由 AutoHotkey 开发，可能被安全软件误报。
