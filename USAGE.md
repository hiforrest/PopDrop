...
- 面板内直接打字即可筛选；多个关键字用半角或全角空格分隔，并按 AND 关系匹配文件名、
  正文、来源名和路径。`/` 或 `Ctrl+F` 聚焦搜索框并全选已有搜索文字，`Esc` 先清空搜索。
- 关闭文本块工作区会结束本次搜索；下次按主快捷键呼出时搜索框和结果列表都恢复初始
  状态，显示全部文本块。
...
- 按住 `Shift` 双击文本块：直接前置发送当前双击的文本块；普通双击仍为快速发送。只有
  单独按住 Shift 时生效，Ctrl、Alt、Win 或组合修饰的双击不会触发发送。
...
Windows Console Host 时，`Enter`、双击、“快速发送”、`Shift+Enter` 和 Shift+双击自动使用终端
...
`Shift+Enter` 和 Shift+双击在终端中与普通快速发送一样粘贴到当前活动窗格，不发送 `Ctrl+Home`。
