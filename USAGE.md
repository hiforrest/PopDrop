...
- Files 工作区没有可见、可命中的固定项分组时，拖入外部纯文件夹会显示双入口：
  左侧约 70% 将文件夹添加为来源，右侧约 30% 将文件夹路径加入固定项。
- 拖入右侧固定项目标只记录路径，不移动、复制或修改文件夹。
- 已有可见固定项分组时，文件仍直接拖到该分组；顶部保持普通工作区栏。
- 拖拽反馈采用按变化刷新，持续按住鼠标移动时不会反复重绘 Tab 和底部信息栏。
...
| 点击右侧“粘贴”图标 | 把非空白剪贴板文字创建为独立文本块并固定 |
| 在卡片区或列表空白处按 `Ctrl+V` | 与“粘贴”图标相同；输入控件中仍为普通粘贴 |
...
右侧图标工具栏按“刷新｜粘贴｜添加/移除固定项｜显示｜设置｜置顶”分组，并用横向分隔
线隔开。所有图标均有悬浮说明；“粘贴”仅在文本工作区且剪贴板含非空白文字时可用。`Ctrl+V` 只在
...
- 面板内直接打字即可筛选；多个关键字用半角或全角空格分隔，并按 AND 关系匹配文件名、
...
- `Ctrl+Enter`：前置发送。将当前选择按普通发送相同的顺序与双换行规则拼接，再投送到
  原输入内容最前方；不会记忆本次动作，也不会自动添加空格、换行或其他分隔符。无法可靠
  恢复原窗口及输入焦点、安全输入框或目标位置已变化时停止自动粘贴，正文保留在剪贴板。
- 按住 `Ctrl` 双击文本块：直接前置发送当前双击的文本块；普通双击仍为快速发送。只有
  单独按住 Ctrl 时生效，Shift、Alt、Win 或组合修饰的双击不会触发发送。动作以第二次鼠标
  按下时的修饰键快照为准；第二次左键抬起后立即切回原窗口，Ctrl 可以继续按住。只有后来
  按下的 Shift、Alt、Win 等额外修饰键需要先释放；等待不阻塞界面，超过 10 秒会取消本次
  动作且不会锁住后续发送。
...
当呼出 PopDrop 前的顶层窗口是 Windows Terminal，或 `ConsoleWindowClass` 且实际宿主为
Windows Console Host 时，`Enter`、双击、“快速发送”、`Ctrl+Enter` 和 Ctrl+双击自动使用终端
兼容分支。识别依据是顶层窗口类与窗口拥有者进程，不是终端中运行的 CMD、PowerShell、
pwsh、SSH、REPL 或全屏程序。首版明确覆盖：

- Windows Terminal（稳定版与可识别的预览宿主）；
- Console Host 承载的 CMD；
- Console Host 承载的 Windows PowerShell。

终端分支按以下顺序处理：

1. 从拼接后的正文绝对开头和结尾删除连续 CRLF、LF、CR，混合换行也按换行 token 处理；
2. 保留内部换行、普通空格、缩进及原文件，不把规范化结果写回文本块；
3. 清理后为空或仅含空白/Unicode 分隔空白时停止；
4. 单行直接投送；多行先运行中文自然语言规则，不满足才显示默认选“否”的确认框；
5. 确认前，剪贴板已经是清理后的终端正文；取消或后续失败都保留这份正文；
6. 重新激活并核对同一宿主后只投送正文，不额外发送回车。

中文免确认规则每次只分析本次多行正文，不读取文件名、路径、哈希，也没有固定文本名单。
全部非空有效行必须是含中文的自然语言行或明确的 Markdown 标题/列表/引用结构；英文
Markdown 标题可以作为结构行，中文句子可夹杂 Windows、Git、AutoHotkey、C++、
`README.md` 等术语。整体至少有四个汉字，且在非标题主体的汉字与拉丁字母中，汉字占比
不得低于 35%。以下任一情况会使整段退出免确认：

- 独立英文行（包括 `git status` 等命令）、主要为英文或未知结构；
- Markdown/脚本代码围栏，或不成对、含空格/操作符的反引号代码片段；
- CMD、PowerShell、类 Unix 命令提示符；
- 独立路径、URL、参数行、变量赋值或调用前缀；
- `&&`、`||`、管道、分号、命令替换、调用、重定向等 Shell 结构。

这套规则只用于减少中文自然语言提示词的重复确认，不分析命令含义，也不承诺内容安全。
Windows Terminal 的 `multiLinePasteWarning` / `largePasteWarning` 仍由 Windows Terminal
决定是否显示，PopDrop 不读取、修改或绕过这些设置。

Console Host 使用一次 `WM_PASTE`，不依赖 CMD/PowerShell 的快捷键。Windows Terminal
没有通用 `WM_PASTE` 接口，因此 PopDrop 只派发一次兼容终端原始输入和括号粘贴模式的
`Shift+Insert` 粘贴动作；若
用户解绑该动作，PopDrop 不会再模拟右键或尝试第二快捷键，以免第一次其实已成功或正在
显示原生警告时发生重复粘贴，正文会继续留在剪贴板供手动处理。权限等级不一致也可能使
激活或输入投送被 Windows 阻止。

终端的顶层宿主无法判断当前活动窗格内究竟是 Shell、REPL、SSH、文本选择还是全屏程序，
因此多行确认不能等同于运行状态判断。终端也没有跨程序通用的“当前输入开头”，所以
`Ctrl+Enter` 和 Ctrl+双击在终端中与普通快速发送一样粘贴到当前活动窗格，不发送 `Ctrl+Home`。
IDE 集成终端和第三方终端不在首版承诺范围，未识别目标继续使用普通窗口既有路径。
...
| `UiScale` | 主面板整体缩放，可选 `100`、`125`、`150`、`175`、`200`；默认 `100`。窗口、控件、文字、缩略图、文本块卡片与间距会同步缩放，并与 Windows 显示缩放叠加。建议通过“设置 → 共享设置 → 界面设置”修改，保存后重启生效；当前不改变设置窗口及独立对话框的整体尺寸。 |

### 内容更新方式

在“PopDrop 设置 → 内容更新方式”中可选择：

- **极速显示（默认推荐）**：日常首选，极致轻快的使用体验。
- **准确优先**：保障内容正确，仅限极速模式异常时使用。

极速显示会持续监听所有工作区的本地来源；多个工作区引用同一文件夹时，变更会分别更新
各自的内存与磁盘快照。固定项变化和后台重试期间会保留上一份完整来源画面，不会只剩固定
...
右侧图标工具栏使用横向分隔线表达操作分组；鼠标悬浮时显示浅蓝背景、蓝色描边及功能
说明。工具栏拥有独立宽度，不覆盖内容列表或原生滚动条。
...
预览窗口底部会在不增加窗口总高度的情况下显示文件名、自动选择 KB/MB 的大小和修改时间；
无法生成预览时会显示文件图标及同样的信息。该信息栏可在设置页“文件显示与过滤 → 文件内容预览”中关闭。
...
| `ShowFileInfo` | `1` 在预览区内显示文件名、大小和修改时间；默认开启。 |
...
PDF 预览默认关闭。可在“设置 → 文件显示与过滤”中勾选“PDF 预览”；组件尚未安装时
...
能力检测失败时 PopDrop 不接管空格键；文本块搜索框聚焦时 Space 也始终保留给文字和
输入法输入，不受鼠标悬停位置影响。成功打开后进入临时 `QuickViewActive`：
temporary 模式不会因外部查看器获得焦点而隐藏；焦点项变化以 220 ms 防抖同步；
当焦点仍在 PopDrop 时，再按 Space 或 Esc 可关闭外部预览，Enter/双击正式打开文件前
也会关闭。Seer 的路径更新满足其 5000 命令至少 200 ms 的调用间隔；悬浮路径在发送前
锁定，不会被旧选中项覆盖。焦点进入 QuickLook 后键盘全部由 QuickLook 处理；焦点位于
Seer 主预览窗口时，Space 由 PopDrop 发送 5005 可靠关闭，而 Seer 的菜单、弹窗和功能
子窗口仍使用原生键盘行为。查看器主窗口、同进程对话框及 owner 子窗口都属于同一预览会话；
操作期间查看器保持在 PopDrop 上方，关闭后焦点返回 PopDrop 原来的列表控件。窗口层级
会在会话结束时恢复，不会修改或保存 PopDrop 的置顶设置。PopDrop 不安装、捆绑或模拟
按键调用任何外部查看器。
...
工作区保存不同的文件来源组合及来源专属设置。主面板顶部使用 Tab 常驻显示工作区；最多
直接显示 8 个，其余收拢到“更多”菜单。选择另一项后立即刷新，只向扫描 worker 发送新
工作区的来源，并显示该工作区自己的固定项；当前工作区来自“更多”时仍会提升为可见 Tab。
...
关闭；已有配置仍沿用用户保存的值。主面板宽度允许 330–980 逻辑像素：窄窗下顶部
工作区继续使用原生左右翻页，超过 8 个工作区的项目继续放入“更多”菜单；近期栏仅在
布局空间不足时临时隐藏，拉宽后自动恢复。主面板重新打开时仍会把超宽值限制到 980，
避免恢复成接近最大化的宽屏状态。
...
右侧最下方的图钉按钮可以实时切换两种常用模式，并立即保存到 `config.ini`：

- 实心图钉及持续浅蓝背景对应 `always_on_top`，失去焦点后仍保持显示。
- 线框图钉对应 `temporary`，失去焦点后自动隐藏。
...
稳定的外部纯文件夹拖入时，普通工具栏会在原位置临时切换为一个整行可命中的
**＋ 松开，将文件夹添加为来源**。放下后按拖拽顺序追加为当前工作区来源，不复制、
移动、创建或删除真实文件夹。文件夹仍可拖到下方固定项、Files 或 Launcher 分组，执行
这些区域原有语义。

内部或外部纯文件拖入时采用同一规则：当前已有可见、可命中的固定项分组时，顶部保持
普通工具栏，用户直接拖入该分组；固定项为空、未渲染或已滚出可命中区域时，顶部才切换为
**☆ 松开，将文件加入固定项**。侧边“＋固定项”按钮不再作为拖拽目标，只保留点击后选择
文件的普通功能。文本块工作区的顶部入口只接受 `.md`、`.txt`。

智能入口覆盖在顶部工作区导航之上，但不会隐藏承载主列表的原生 Tab 控件；拖拽期间仍可
直接把文件或文件夹投放到固定项、Files、Launcher 或文本来源分组。入口在内容区右边缘
结束，右侧刷新、粘贴、展开/收起、添加/移除、显示、设置和置顶按钮始终保持可见、可点击。

文件和文件夹混合选择不会显示智能入口，也不会自动拆分；状态栏提示分别拖入。虚拟文件、
URL、网页图片、特殊 Shell 对象、无法可靠判断的对象和 PopDrop 内部拖拽同样保持普通
工具栏，下方已有区域仍按原规则处理。`Ctrl`、`Shift` 不改变顶部入口语义；顶部只向
OLE 来源返回 `COPY` 或 `NONE`，绝不返回 `MOVE`。
...
来源文件夹标题前的 `⋁` 表示当前展开，`⋀` 表示当前收起。左键单击来源标题会切换
折叠状态：收起后只保留标题并隐藏该来源的全部图标，再次单击即可展开。状态按工作区和
来源分别记录，并在本次运行的刷新、切换工作区和热视图重建期间保持；重启后恢复展开。

在 Files 或 Launcher 来源的分组标题上右键，会显示独立的来源管理菜单：

- **选择窗口定位到此**：仅当本次 PopDrop 是从 Windows 标准打开文件、保存/另存为、
  选择文件或选择文件夹窗口呼出时显示，并位于菜单首项。它只把原选择窗口导航到该来源
  路径，不改“文件名”字段、不操作剪贴板，也不自动点击打开、保存或选择。来源标题右侧
  同时显示同名原生链接，可直接单击。现代 Explorer 风格、旧式树形文件夹选择器和 Inno
  Setup 自有的 `TSelectFolderForm` 均支持；自定义窗还需同时具备树控件、唯一可见路径输入框
  以及精确窗体类或明确的文件夹选择标题。普通设置框不会仅凭标题或“确定”按钮被启用。
...
`Shift` 反转规则不变；左键来源标题切换折叠，打开来源文件夹继续使用右键菜单。固定项标题和空工作区占位不会
...
| 内部或外部纯文件，且没有可见固定项分组 | 顶部「☆ 加入固定项」 | 新批次置于当前工作区固定项最前；不操作真实文件 |
...
| 任意有效本地文件或文件夹 | 固定项分组或侧边「＋ 固定项」按钮 | 只加入固定项 |
...
拖到固定项分组只记录路径，不复制、不移动、不删除真实文件；只要该分组可见、可命中，
内部或外部纯文件拖拽都不显示顶部入口。侧边「＋ 固定项」仅用于点击选择文件，不接收
拖放。重复路径继续跳过且不改变已有位置，新加入的一批保持原顺序并置于最前。
...
顶部智能入口，对同步、纯 `CF_HDROP`、且没有 URL、虚拟文件或图片格式信号的稳定本地
对象读取一次路径列表。路径缓存在当前拖拽 Session 中，`Drop` 不会再次调用
...
主面板底栏固定为 42 逻辑像素高。左侧文件路径和操作反馈占用右侧下载入口之外的全部
宽度，最多显示两行并垂直居中，第三行截断；项目总数不再单独占栏。右侧入口固定为
84 逻辑像素，并以“↓下载”“↓2 35%”“!失败1”“✓完成”等短状态显示任务概况。
任务持续超过约 300 ms 后才切换为整体任务数/进度，并至少保留约 800 ms；完成提示
显示约 3 秒后恢复，未查看的失败保持提示。来源标题显示该目标的接收项数或真实百分比。
...
点击「＋ 固定项」可以选择一个或多个文件，但该按钮不接收拖放。内部或外部纯文件通过
顶部智能入口加入固定项；文件夹可以拖到固定项分组，作为单独固定项加入，不会展开或
添加其中的内容。拖到 Files/Launcher 来源时则按上一节的来源投放规则执行。
...
PopDrop 会拒绝操作，避免来源路径在配置中失效。重命名文件时，文件名与扩展名使用同一
行中的独立文本框，修改主体名称时不必手工保留扩展名；扩展名框不输入句点。

按 `Delete` 或从 PopDrop 右键菜单选择“删除”会立即把全部所选项目移入回收站，不弹出
二次确认；回收站不可用时不会改为永久删除。按 `Shift + Delete` 会先显示不可撤销警告，
仅在确认后永久删除全部所选项目。
...
在“PopDrop 设置 → 共享设置 → 文件打开与操作”的“文件管理器”区域中，可以选择：
...
在“PopDrop 设置 → 共享设置 → 文件打开与操作”的“应用与工具操作”区域中，可以添加
...
也可以在“PopDrop 设置 → 共享设置 → 通用 → 开机启动”中勾选“登录 Windows 后自动启动 PopDrop”。
PopDrop 会在当前用户的启动文件夹维护快捷方式；取消勾选会自动移除。启动时即使快捷方式位于
系统启动文件夹，配置和缓存也不会写入该文件夹。
