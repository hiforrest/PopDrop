...
| `CacheCleanupEnabled` | 是否自动管理运行时缓存；默认开启，启动时整理并每小时复查。关闭后仍可在设置页手动“立即整理”。 |
| `CacheRetentionDays` | 损坏数据库备份及已删除工作区孤立快照的保留期，范围 1～90 天，默认 7 天；损坏数据库备份同时最多保留 3 组。 |
...
- 每次启动立即执行一次，并在运行期间每小时复查；正常退出会同时终止和回收当前扫描与
  非当前工作区扫描任务。
- `request-*`、`ready-*`、`inactive-*` 仅在不属于当前任务且超过 15 分钟后删除，避免误伤
  刚启动或仍可能写入的 Worker；`.writing` 等中间文件保留 1 小时安全窗口。
- `index.db.corrupt-*` 默认保留 7 天且最多 3 组；已删除工作区留下的 `workspace-*.ini`
  按设置的保留期回收，SQLite 内的孤立工作区记录会直接从事务索引中清除。
- `index.db-wal` 与 `index.db-shm` 是 SQLite 正常事务文件，程序运行时不会删除；WAL 使用
  1 MB 日志尺寸目标和自动检查点，正常退出时执行截断检查点。
- `preview-cache-v1` 继续由隔离预览组件管理，并遵守 `[Preview]` 下的容量、项目数与未引用
  天数上限。两套维护器均只接受 PopDrop 自有且名称严格匹配的文件，不扫描或删除用户文件。
- 每次整理结果写入 `cache-maintenance.ini`。也可在“设置 → 内容更新方式 → 缓存管理”查看
  当前占用、修改保留期、立即整理或打开缓存目录。
