...
- 筛选只作用于对应文件夹的扫描结果，不影响固定项目和 Windows 最近打开侧边栏。
...
点击「添加文件」选择文件，它们会出现在面板的固定区域。也可以从资源管理器把一个或多个文件、文件夹直接拖进 PopDrop 窗口；文件夹会作为单独的固定项目加入，不会展开或添加其中的内容。

固定列表继续保存在 `config.ini` 的 `[PinnedFiles]` 里，与旧配置完全兼容。选择固定项后点击「取消固定」，只移除面板里的记录，不会影响原文件或文件夹。双击固定文件夹会在资源管理器中打开。
